...
<body ng-app="youlexuan" ng-controller="pageController" ng-init="loadSku()">
{{sku.spec}}
							...
							<#list specificationList as spec>
								<dl>
									...
									<dd>
										...
										<a ng-click="updateSpec('${spec.attributeName}','${item}')" href="javascript:void(0);" class="{{select('${spec.attributeName}','${item}') ? 'selected' : ''}}">${item}<span title="点击取消选择">&nbsp;</span>
										</a>
										</#list>
									</dd>
								</dl>
