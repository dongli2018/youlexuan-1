...
public class GoodsController {
	...
	public Result update(@RequestBody Goods goods){
		try {
			...
			//删除solr
			//通过消息中间件删除solr的内容
			jmsTemplate.convertAndSend(activeMQQueue,new Long[]{goods.getGoods().getId()});
			//itemSearchService.deleteDate(new Long[]{goods.getGoods().getId()});
			//删除HTML静态页面
			jmsTemplate.convertAndSend(activeMQTopic,new Long[]{goods.getGoods().getId()});
	...
	public Result delete(Long [] ids){
		try {
			...
			//通过消息中间件删除solr的内容
			jmsTemplate.convertAndSend(activeMQQueue,ids);
			////删除HTML静态页面
